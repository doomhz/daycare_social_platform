(function() {
  ...
  Kin.Staff.InvitesView = (function(_super) {
    ...
    InvitesView.prototype.tplUrl = '/templates/main/staff/invites.html';
    ...
    InvitesView.prototype.sendInvite = function(ev) {
      var $form, staffData, that;
      ev.preventDefault();
      that = this;
      $form = $(ev.target);
      staffData = this.getFormData($form, "staff");
      if (staffData) {
        if (this.isUniqueEmail(staffData.email)) {
          return this.saveFriendRequest(staffData);
        } else {
          return dAlert("An invite has already been sent to " + staffData.email + ".<br />Please choose another one.");
        }
      }
    };
